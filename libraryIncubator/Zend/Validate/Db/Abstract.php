...
abstract class Zend_Validate_Db_Abstract extends Zend_Validate_Abstract
{
     ...
     */
    public function __construct($table, $field, $exclude = null, $adapter = null)
    {
        if ($adapter!==null) {
    ...
    {
        require_once 'Zend/Validate/Db/Table.php';
        $config = array(Zend_Db_Table::ADAPTER => $this->_adapter,
                        Zend_Db_Table::NAME => $this->_table);

        /**
         * Instantiate the table
         */
        $table = new Zend_Validate_Db_Table($config);

        /**
         * Re-set _adapter incase one was not specified.
         */
         ...
         * Build select object
         */
        $select = $table->select($this->_field)->where($adapter->quoteIdentifier($this->_field).' = ?', $value);
        if ($this->_exclude !== null) {
            if (is_array($this->_exclude)) {
                $select->where($adapter->quoteIdentifier($this->_exclude['field']).' != ?', $this->_exclude['value']);
            } else {
                $select->where($adapter->quote($this->_exclude));
            }
        }
        $select->limit(1);
