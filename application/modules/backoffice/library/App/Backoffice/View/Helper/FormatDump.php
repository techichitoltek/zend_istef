...
class App_View_Helper_FormatDump extends Zend_View_Helper_Abstract
{
    ...
    protected function _displayArrayDump($array){
        ...
        $output .= '<div class="toggle pre">' . Zend_Debug::dump($array, NULL, FALSE) . '</div>';
        return $output;
    }
    ...
    protected function _displayObjectDump($object){
        $output = $this->view->displayClass($object) . ' (<a href="">view details</a>)';
        $output .= '<div class="toggle pre">' . Zend_Debug::dump($object, NULL, FALSE) . '</div>';
        return $output;
    }
}